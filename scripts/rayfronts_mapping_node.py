...
try:
	...
	from rayfronts.mapping import OccupancyVDBMap
...
class SemanticDepthOctoMapNode(Node):
	"""Simplified semantic depth VDB mapping node using RayFronts OccupancyVDBMap."""
	...
	def __init__(self):
		...
		# Initialize VDB mapper for occupancy
		...
		self.get_logger().info(f"   Mapper: VDB OccupancyMap (OpenVDB)")
	...
	def _initialize_vdb_mapper(self):
		...
		try:
			...
			# Noise-robust parameters for depth sensors
			self.vdb_mapper = OccupancyVDBMap(
				...
				max_depth_sensing=1.5,  # CRITICAL: Automatic depth noise reduction (1.5x std dev)
			...
			self.get_logger().info(f"VDB OccupancyMap initialized (device: {self.vdb_mapper.device})")
			self.get_logger().info(f"Noise-robust settings:")
			self.get_logger().info(f"   Auto depth filtering: 1.5σ noise reduction")
	...
	def _process_hotspot_with_depth(self, mask: np.ndarray, pose: PoseStamped, depth_m: np.ndarray,
		...
		try:
			...
			# Update VDB map with hotspot points for semantic voxels
			if is_narration:
			...
			self.get_logger().info(
				...
				f"(rgb_ts={rgb_ts:.6f}, depth_ts={used_ts[0]}, pose_ts={used_ts[1]}, type={hotspot_type})"
			)
			return True
		...
		try:
			...
			with self.semantic_voxels_lock:
				for voxel_key, semantic_info in self.semantic_voxels.items():
					voxel_position = semantic_info['position']
